...
with gr.Blocks(theme=theme) as demo:
    gr.Markdown(
        """
        # Graph RAG Technical Support Assistant

        This AI agent combines **knowledge graph traversal** with **vector search** to answer
        questions about networking products, their features, errors, and solutions.

        **Try multi-hop questions** like "What solutions exist for errors related to the Router X500?"
        that require traversing entity relationships the graph captures.
        """
    )
    ...
    with gr.Row():
        textbox = gr.Textbox(
            placeholder="Ask about products, errors, solutions, or features...",
            container=False,
            scale=7
        )
    ...
    gr.Examples(
        examples=[
            ...
            ["How do I configure WiFi 6E on the Router X500?"],
            ["Which products share the VLAN support feature?"],
            ["What is ERR-012 and how do I fix it?"],
            ["What errors can occur on the Switch SW-3000?"]
        ],
        inputs=[textbox]
    )
