...
class LangGraphResponsesAgent(ResponsesAgent):
    def __init__(
        self,
        uc_tool_names: Sequence[str] = ("main_build.dbdemos_graph_rag.*",),
        ...
        # Load Unity Catalog tools (includes graph traversal functions)
        self.tools: list[BaseTool] = UCFunctionToolkit(function_names=list(uc_tool_names)).tools

        # Add retriever if configured (for vector search)
        if retriever_config:
            self.tools.append(
                VectorSearchRetrieverTool(
...
# ==========================
model_config = ModelConfig(development_config="../03-agent-eval/agent_config.yaml")
